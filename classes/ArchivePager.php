...
class ArchivePager
{
    ...
    protected function setupMonthPager()
    {
        ...
        if ($first_date->getTimestamp() < $current_date->getTimestamp()) {
            ...
            $this->previous_text = $previous_date->format('F') . ', ' . $previous_date->year;
            $this->previous_url = $this->makePagerUrl(array(
                'year' => $previous_date->year,
                'month' => $previous_date->month,
            ));
        } else {
            $this->previous_text = $current_date->format('F') . ', ' . $current_date->year;
            $this->previous_url = '';
        }
        // next
        if (($this->current_year < date('Y')) || ($this->current_month < date('m'))) {
            ...
            $this->next_text = $next_date->format('F') . ', ' . $next_date->year;
            $this->next_url = $this->makePagerUrl(array(
                'year' => $next_date->year,
                'month' => $next_date->month,
            ));
        } else {
            $this->next_text = $current_date->format('F') . ', ' . $current_date->year;
            $this->next_url = '';
        }
    }
